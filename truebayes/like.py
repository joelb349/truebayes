...
def synlike(a, syntrain, iterations=1000000):
    with torch.no_grad():
        ...
        while cnt < iterations:
            ...
            alphareal, alphaimag = alpha[:,:], 0
            ##alphaimag = alpha[:,0::2], alpha[:,1::2]
            ...
            loglike = alphareal @ areal - 0.5*alphanorm.unsqueeze(1) - 0.5*anorm
            print(loglike)
            if adapt is None:
            ...
            like += torch.nan_to_num(torch.t(cpxt) @ torch.exp(loglike))
